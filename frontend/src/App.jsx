              ...
              {viewMode === 'CUSTOM_INPUT' ? (
                ...
                >
                  {jobLoading ? (
                    ...
                    <>
                      {/* Job Card */}
                      <div key={`job-${currentJob.id}`} className="animate-slideUpFade relative">
                        {isCustomResult && (
                          <div className="mb-5">
                            <button
                              onClick={handleOpenCustomInspector}
                              className="group inline-flex items-center gap-2 px-4 py-2 text-[15px] font-bold text-indigo-700 bg-indigo-50 hover:bg-indigo-100 rounded-full transition-all cursor-pointer shadow-sm border border-indigo-100/50 hover:border-indigo-200"
                            >
                              <svg className="transition-transform group-hover:-translate-x-1" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M19 12H5" /><path d="M12 19l-7-7 7-7" /></svg>
                              {lang === 'en' ? 'Scan Another JD' : 'Quay lại màn hình quét'}
                            </button>
                          </div>
                        )}
                        <JobCard
              ...
              <ul className="space-y-3">
                ...
                <motion.li whileHover={{ x: 2 }} className="flex items-start gap-2.5 text-xs text-slate-700 bg-white/60 p-2.5 rounded-xl border border-slate-200/50">
                  ...
                  <div>
                    ...
                    <span className="opacity-80">{lang === 'en' ? 'Flag recruiters migrating to unverified messaging apps (Telegram, WhatsApp).' : 'Cảnh giác recruiter chỉ trao đổi qua ứng dụng không chính thức.'}</span>
                  </div>
                </motion.li>
              </ul>
            </motion.div>
