...
* MEDIUM RISK (Yellow Flags / Cảnh báo nhẹ): The job appears to be from a LEGITIMATE company (e.g., FPT, Vingroup, PECC2), but contains strict terms, minor logical anomalies, or potential phishing indicators that candidates should be aware of. Examples: A binding training contract (ràng buộc đào tạo 3-6 tháng đền bù chi phí), a minor contact mismatch (e.g., working in Nha Trang but sending CV to an HCM email), or typos/misspellings in professional email addresses/domains (e.g., 'talent-accquisition' instead of 'talent-acquisition') which could indicate impersonation. When generating flags for a MEDIUM risk post, \`category_vi\` should be 'LƯU Ý' or 'CẢNH BÁO', \`category_en\` should be 'CAUTION' or 'WARNING', and the \`reason\` MUST clarify the concern.
